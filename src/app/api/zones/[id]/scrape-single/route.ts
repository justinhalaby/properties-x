...
// POST /api/zones/[id]/scrape-single - Scrape a single building
  ...
  try {
    ...
    if (!matricule) {
      return NextResponse.json(
        { error: "Missing required field: matricule" },
        { status: 400 }
      );
    }
    ...
    if (existing) {
      return NextResponse.json(
        { error: "Building already scraped", already_scraped: true },
        { status: 409 }
      );
    }
    ...
    // Update zone stats
    await fetch(`${request.url.split('/api')[0]}/api/zones/${zoneId}/stats`);
