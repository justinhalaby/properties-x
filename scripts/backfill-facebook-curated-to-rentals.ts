/**
 * Facebook Pipeline Stage 2: CURATED → RENTALS
 * Transforms FacebookRentalCurated records to rentals table
 */
...
async function main() {
  console.log('\n🚀 Facebook: CURATED → RENTALS Transformation');
  console.log('===============================================\n');
  ...
  for (let i = 0; i < metadataRecords.length; i++) {
    ...
    try {
      ...
      const { rentalInput, warnings, errors } = transformCuratedToRental(curated as FacebookRentalCurated);
      ...
      // Use already-downloaded images/videos from metadata
